...
class OrderInfoMapper(object):
	...
	def selectBatch(self, start, number):
		...
		return self.session.query(OrderInfoDO).filter(OrderInfoDO.isDeleted=="N").\
				   ...
				   filter(OrderInfoDO.orderSn.notilike("%S%")).filter(OrderInfoDO.orderAmount>0).filter(OrderInfoDO.gmtCreate<'2016-01-01 00:00:00').order_by(OrderInfoDO.id)[start:start+number]
