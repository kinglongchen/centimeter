...
class SaleReturnExchangeBillMapper(object):
	...
	def selectReturnGoods4PartReject(self, outOrderSnList):
		return self.session.query(SaleReturnExchangeBillDO).filter(SaleReturnExchangeBillDO.id.in_([27,28]))\
			...
			.filter(SaleReturnExchangeBillDO.mainBillNo.in_(outOrderSnList))
